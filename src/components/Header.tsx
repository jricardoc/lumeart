...
export default function Header() {
  ...
  return (
    ...
    >
      {/* Scroll progress bar — gradient gold→purple */}
      <div
          ...
          <nav className="hidden md:flex items-center gap-6 lg:gap-8">
            {navLinks.map((l) => (
              <a
                key={l.href}
                href={l.href}
                className={`relative text-[11px] lg:text-xs uppercase tracking-[0.18em] font-medium transition-colors duration-300
      ...
      {/* Mobile fullscreen menu — HUD / Sci-Fi */}
      {open && (
        <div className="md:hidden fixed inset-0 top-16 z-40 overflow-hidden">
          {/* Layered background */}
          ...
          <div
            ...
            style={{
              backgroundImage:
                'radial-gradient(circle, #d4a853 0.8px, transparent 0.8px)',
          ...
          {/* Decorative rotating crosshair */}
          <div className="absolute top-8 right-8 w-16 h-16 opacity-[0.06] pointer-events-none">
            <div
          ...
          <div className="relative px-6 pt-6 pb-2">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="font-pixel text-[7px] text-gold/30 tracking-[0.4em] uppercase">
                Navigation System
              </span>
            </div>
          ...
          {/* Navigation links */}
          <nav className="relative flex flex-col px-6 pt-4 gap-1">
            {navLinks.map((l, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Terminal index */}
                  <span className={`font-mono text-[10px] w-5 shrink-0 transition-colors duration-300 ${
                  ...
                  {/* Label */}
                  <span className={`font-heading text-sm uppercase tracking-[0.2em] transition-all duration-300 ${
                    isActive
                      ? 'text-gold font-semibold'
                      : 'text-gray-300 group-hover:text-gold font-medium'
                  }`}>
                    {l.label}
                  </span>

                  {/* Arrow on hover / active */}
                  <span className={`ml-auto font-mono text-[10px] transition-all duration-300 ${
                    isActive
                      ? 'text-gold/40 translate-x-0'
                      : 'text-transparent group-hover:text-gold/25 -translate-x-2 group-hover:translate-x-0'
                  }`}>
                    →
                  </span>
                </a>
          ...
          <div className="absolute bottom-0 left-0 right-0 px-6 pb-8">
            ...
            >
              ...
              <div>
                <span className="text-xs text-gray-300 group-hover:text-gold transition-colors font-medium block">
                  @lume.art3d
                </span>
            ...
            <div className="flex items-center justify-between mt-5">
              <span className="font-pixel text-[6px] text-gray-700 tracking-widest">
                SYS.OK
              </span>
          ...
          {/* Corner brackets */}
          <div className="absolute top-4 left-4 w-5 h-5 border-l border-t border-gold/10" />
          <div className="absolute top-4 right-4 w-5 h-5 border-r border-t border-gold/10" />
          <div className="absolute bottom-4 left-4 w-5 h-5 border-l border-b border-gold/10" />
          <div className="absolute bottom-4 right-4 w-5 h-5 border-r border-b border-gold/10" />
        </div>
      )}
    </header>
  );
